...
import static org.wso2.carbon.extension.identity.verification.provider.util.IdVProviderMgtConstants.IDVP_SECRET_TYPE_IDP_SECRETS;
...
public class IdVPSecretProcessor implements SecretsProcessor<IdentityVerificationProvider> {
    ...
    public IdentityVerificationProvider decryptAssociatedSecrets(IdentityVerificationProvider idVProvider)
        ...
        for (IdVConfigProperty idVConfigProperty : clonedIdVP.getIdVConfigProperties()) {
            ...
            String secretName = buildSecretName(clonedIdVP.getId(), idVConfigProperty.getName());
            if (secretManager.isSecretExist(IDVP_SECRET_TYPE_IDP_SECRETS, secretName)) {
                ResolvedSecret resolvedSecret =
                        secretResolveManager.getResolvedSecret(IDVP_SECRET_TYPE_IDP_SECRETS, secretName);
    ...
    public IdentityVerificationProvider encryptAssociatedSecrets(IdentityVerificationProvider idVProvider)
        ...
        for (IdVConfigProperty idVConfigProperty : clonedIdVP.getIdVConfigProperties()) {
            ...
            if (secretManager.isSecretExist(IDVP_SECRET_TYPE_IDP_SECRETS, secretName)) {
                ...
                addNewIdpSecretProperty(secretName, idVConfigProperty);
                idVConfigProperty.setValue(buildSecretReference(secretName));
            }
        }
    ...
    public void deleteAssociatedSecrets(IdentityVerificationProvider idVProvider) throws SecretManagementException {
        ...
        for (IdVConfigProperty idVConfigProperty : idVProvider.getIdVConfigProperties()) {
            ...
            if (secretManager.isSecretExist(IDVP_SECRET_TYPE_IDP_SECRETS, secretName)) {
                secretManager.deleteSecret(IDVP_SECRET_TYPE_IDP_SECRETS, secretName);
            }
        }
    }
    ...
    private String buildSecretReference(String secretName) throws SecretManagementException {
        ...
        SecretType secretType = secretManager.getSecretType(IDVP_SECRET_TYPE_IDP_SECRETS);
        return secretType.getId() + ":" + secretName;
    }

    private void addNewIdpSecretProperty(String secretName, IdVConfigProperty idVConfigProperty)
            throws SecretManagementException {

        Secret secret = new Secret();
        secret.setSecretName(secretName);
        secret.setSecretValue(idVConfigProperty.getValue());
        secretManager.addSecret(IDVP_SECRET_TYPE_IDP_SECRETS, secret);
    }
    ...
    private void updateExistingSecretProperty(String secretName, IdVConfigProperty idVConfigProperty)
        ...
        if (!resolvedSecret.getResolvedSecretValue().equals(idVConfigProperty.getValue())) {
            secretManager.updateSecretValue(IDVP_SECRET_TYPE_IDP_SECRETS, secretName, idVConfigProperty.getValue());
        }
    }
}
